  ...
  pacman::p_load('shiny','shinythemes','shinyWidgets','tidyverse','geobr',
               'sf','readr','openxlsx','leaflet',"leaflet.extras","metan")
      ...
      tabPanel(
        ...
        fluidPage(
            mainPanel(h1("Bem vindo ao analisador de dados das Eleições"),
                    h4("Bem-vindo, com o objetivo de analisar os dados disponibilizados pelo TSE, desenvolvemos essa plataforma que permite observar a disposição geográfica dos votos, durante e após as eleições, a diferença de votos percentuais obtida por um candidato entre o 1º e 2º turno, e uma projeção dos votos, sendo estimado um resultado para a eleição presidencial. "),
  
  h4("Os dados são observados por um gráfico tendo como base o mapa brasileiro, de maneira que o observador pode selecionar vê-lo com os atuais dados municipais ou estaduais, sendo o Distrito-Federal uma exceção onde as regiões administrativas serão observadas como municípios e o seu todo como um estado. Os resultados podem ser observados tanto para o 1º como para o 2º turno das eleições. "),
  
  h4("A diferença de dados é um comparativo para o crescimento ou decrescimento do número de votos, para os candidatos que participarem de um segundo turno, o cálculo é realizado levando em consideração o número de votos que cada candidato obteve no primeiro turno, de maneira que um candidato pode ganhar mais votos entre o primeiro e segundo turno, mas o crescimento percentual ser menor do que o de seu adversário nas urnas."),
  
   h4("	A projeção dos dados é realizada uma vez que temos pelo menos 2% dos votos de cada município, a porcentagem dos votos de cada candidato é inferida para todos os eleitores do município, permitindo observar a projeção de um possível candidato eleito para a presidência, qualquer resultado obtido não tem nível de confiança 100%, ou seja, é apenas uma aproximação do resultado logo não nos responsabilizamos por uma possível divergência entre o resultado projetado e observado.")
          ))
      ),
  ...
  server <- function(input, output, session) {
    ...
    dados_mapa <- reactive({ 
      ...
      tratamento(mapa_db, input$area, diff = diff())
    ...
    output$mapa <- renderLeaflet({
      mapa(dados_mapa(),input$turno, diff = diff())
      # if(input$turno=="Diferença"){
      #   mapa(dados_mapa(),input$turno, diff = diff)
      # }
